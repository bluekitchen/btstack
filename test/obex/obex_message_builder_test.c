...
TEST(OBEX_MESSAGE_BUILDER, CreateConnectWithHeaderTarget){
    ...
    status = obex_message_builder_header_add_target(actual_message, actual_message_len, service_uuid, sizeof(service_uuid));
...
TEST(OBEX_MESSAGE_BUILDER, CreateConnectWithHeaderApplicationParameters){
    ...
    status = obex_message_builder_header_add_application_parameters(actual_message, actual_message_len, &application_parameters[0], sizeof(application_parameters));
    expected_message[2] += header_len;
    validate_success(expected_message, expected_message[2]);
}
