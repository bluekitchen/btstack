...
static void stm32f407_flash_read(void *dst, uint32_t addr, uint32_t len)
{
    memcpy(dst, (void *)((uint32_t *)addr), len);
}

static int stm32f407_flash_write(void *src, uint32_t addr, uint32_t len)
{
	unsigned int i;
	HAL_FLASH_Unlock();
	for (i = 0;i < len;i+=4){
		HAL_FLASH_Program(FLASH_TYPEPROGRAM_WORD, addr + i, *(uint32_t *)(src+i));
	}
...
static int stm32f407_flash_erase(uint32_t addr, uint32_t len)
{
    uint32_t sector;
	FLASH_EraseInitTypeDef eraseInit;

    sector = stm32f407_get_sector_from_address(addr);
                    ...
                    uint32_t len)
{
    ...
    if ( dst != NULL &&  len > 0) {
        stm32f407_flash_read(dst, read_addr, len);
    }
