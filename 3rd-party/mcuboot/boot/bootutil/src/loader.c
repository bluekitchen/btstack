...
boot_copy_image(struct boot_loader_state *state, struct boot_status *bs)
{
    ...
    BOOT_LOG_INF("Copying the secondary slot to the primary slot: 0x%x bytes", size);
                              ...
                              struct boot_status *bs)
{
     ...
     */
     /*比较primary slot & secondary slot的sector number & size是否匹配*/
...
context_boot_go(struct boot_loader_state *state, struct boot_rsp *rsp)
{
     ...
     * completed.
     */
     /*第一次遍历，遍历所有的image，确定其swap 类型，以及完成所有之前被中断的swap*/
     ...
     */
     /*第三次遍历，此时所有的image都交换完成，在执行primary slot中的image之前，
       需要对其先验签下*/
    IMAGES_ITER(BOOT_CURR_IMG(state)) {
        if (BOOT_SWAP_TYPE(state) != BOOT_SWAP_TYPE_NONE) {
...
{
    fih_int fih_rc = FIH_FAILURE;
    BOOT_LOG_INF("enter boot_go func");
