...
tBleStatus hci_set_controller_to_host_flow_control(uint8_t Flow_Control_Enable)
{
  ...
  tBleStatus status = 0;
  int index_input = 0;
  cp0->Flow_Control_Enable = htob(Flow_Control_Enable, 1);
  index_input += 1;
  Osal_MemSet(&rq, 0, sizeof(rq));
  rq.ogf = 0x03;
  rq.ocf = 0x031;
                                ...
                                uint16_t Host_Total_Num_Synchronous_Data_Packets)
{
  ...
  tBleStatus status = 0;
  int index_input = 0;
  cp0->Host_ACL_Data_Packet_Length = htob(Host_ACL_Data_Packet_Length, 2);
  index_input += 2;
  cp0->Host_Synchronous_Data_Packet_Length = htob(Host_Synchronous_Data_Packet_Length, 1);
  index_input += 1;
  cp0->Host_Total_Num_ACL_Data_Packets = htob(Host_Total_Num_ACL_Data_Packets, 2);
  index_input += 2;
  cp0->Host_Total_Num_Synchronous_Data_Packets = htob(Host_Total_Num_Synchronous_Data_Packets, 2);
  index_input += 2;
  Osal_MemSet(&rq, 0, sizeof(rq));
  rq.ogf = 0x03;
  rq.ocf = 0x033;
