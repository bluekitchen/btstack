...
static IOReturn darwin_get_pipe_properties(struct darwin_interface *cInterface, uint8_t pipe, darwin_pipe_properties_t *out) {
  ...
  if (get_interface_interface_version() >= 550) {
    ...
    // CRITICAL WORKAROUND for Apple Silicon macOS IOUSBHostFamily kernel panic:
    // Calling GetEndpointPropertiesV3 on Isochronous endpoints of Full-Speed devices
    // dereferences a NULL companion descriptor pointer at offset 0x3 (FAR: 0x3).
    // For isochronous endpoints, fall through to the safe legacy GetEndpointProperties API below!
    if (pipe_properties.bTransferType != kUSBIsoc) {
