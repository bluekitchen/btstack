...
STCubeMX was used to provide the HAL, initialize the device, and create an initial CMakeLists.txt for CMake. The CMake project can be built from the command line or used with most modern IDEs, e.g. JetBrains CLion, Visual Studio Code with the STM32CubeIDE Extension. In addition, SEGGER Ozone project files are generated for debugging.
...
To build all examples with `Ninja`, run `cmake` in the port folder:
    ...
    $ cd port/stm32-f4discovery-cyw55310   
...
In addition, there are currently (December 2025) no drivers for AMR64. 
...
When compiled with "ENABLE_SEGGER_RTT" enabled in `port/btstack_config.h`, the debug console shows up in the Terminal window of Ozone.
You might need to enable "Capture RTT" via the Terminal window context menu. 

## Development with STM32CubeIDE for Visual Studio Code
.. TODO .. 
...
All debug console can be either send via SEGGER RTT or via USART2. To get the console from USART2, connect PA2 (USART2 TX) of the Discovery board to an USB-2-UART adapter and open a terminal at 115200.
...
The port downloads the latest PatchRAM for the CYW55310 from the BlueKitchen Web server.
